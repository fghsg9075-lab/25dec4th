...
export const AdminDashboard: React.FC<Props> = ({ onNavigate, settings, onUpdateSettings, onImpersonate, logActivity }) => {
  ...
  useEffect(() => {
      if (!['SYLLABUS_MANAGER', 'CONTENT_PDF_FREE', 'CONTENT_PDF_PREMIUM', 'CONTENT_PDF_ULTRA', 'CONTENT_VIDEO', 'CONTENT_MCQ', 'CONTENT_TEST', 'CONTENT_NOTES'].includes(activeTab)) {
          setSelSubject(null);
...
<html lang="en">
  ...
  const saveChapterContent = () => {
      ...
      const newData = {
          ...
          ultraLink: editConfig.ultraLink,
  ...
  return (
    <div className="pb-20 bg-slate-50 min-h-screen">
      ...
      {activeTab === 'DASHBOARD' && (
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 mb-6 animate-in fade-in">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                  ...
                  <DashboardCard icon={FileText} label="Normal PDF" onClick={() => setActiveTab('CONTENT_PDF_FREE')} color="cyan" />
                  <DashboardCard icon={FileText} label="Premium PDF" onClick={() => setActiveTab('CONTENT_PDF_PREMIUM')} color="purple" />
                  <DashboardCard icon={FileText} label="Ultra PDF" onClick={() => setActiveTab('CONTENT_PDF_ULTRA')} color="amber" />
      ...
      {['CONTENT_PDF_FREE', 'CONTENT_PDF_PREMIUM', 'CONTENT_PDF_ULTRA', 'CONTENT_VIDEO', 'CONTENT_MCQ', 'CONTENT_TEST', 'CONTENT_NOTES'].includes(activeTab) && (
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 animate-in slide-in-from-right">
              <div className="flex items-center gap-4 mb-6 border-b pb-4">
                  <button onClick={() => setActiveTab('DASHBOARD')} className="bg-slate-100 p-2 rounded-full hover:bg-slate-200"><ArrowLeft size={20} /></button>
                  <h3 className="text-xl font-black text-slate-800">
                      {activeTab.startsWith('CONTENT_PDF') ? 'PDF Study Material' : activeTab === 'CONTENT_VIDEO' ? 'Video Lectures' : activeTab === 'CONTENT_MCQ' ? 'Practice MCQs' : activeTab === 'CONTENT_NOTES' ? 'Rich Text Notes (HTML)' : 'Weekly Tests - Multi-Subject'}
                  </h3>
              </div>
...
Capital of India?       Mumbai  Delhi   Kolkata Chennai 2       Delhi is the capital`}
                      ...
                      {selChapters.map((ch) => (
                          <div key={ch.id} className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-200">
                              ...
                              <button onClick={() => loadChapterContent(ch.id)} className="px-4 py-2 bg-blue-100 text-blue-700 font-bold rounded-lg text-xs hover:bg-blue-200">
                                  {activeTab.startsWith('CONTENT_PDF') ? 'Edit Links' : activeTab === 'CONTENT_NOTES' ? 'Edit Notes' : 'Manage Questions'}
                              </button>
                          </div>
                      ))}
                  </div>
              ...
              {editingChapterId && activeTab !== 'CONTENT_TEST' && (
                  <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 animate-in slide-in-from-right">
                      ...
                      {/* SHARED PRICE FIELD */}
                      <div className="bg-yellow-50 p-4 rounded-xl border border-yellow-200 mb-6 flex items-center justify-between">
                          <div>
                              <p className="text-xs font-bold text-yellow-800 uppercase">Unlock Cost</p>
                              <p className="text-[10px] text-yellow-600">Cost for students to open this item.</p>
                          </div>
                          <div className="flex items-center gap-2">
                              <span className="font-bold text-yellow-700">Coins:</span>
                              <input 
                                  type="number" 
                                  value={editConfig.price || 0} 
                                  onChange={e => setEditConfig({...editConfig, price: Number(e.target.value)})} 
                                  className="w-20 p-2 border border-yellow-300 rounded text-center font-black text-slate-800 bg-white" 
                              />
                          </div>
                      </div>

                      {/* PDF EDITOR (SPLIT BY TYPE) */}
                      {activeTab === 'CONTENT_PDF_FREE' && (
                          <div className="space-y-6">
                              <div>
                                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Free/Normal PDF Link</label>
                              ...
                              <button onClick={saveChapterContent} className="w-full bg-blue-600 text-white font-bold py-3 rounded-xl shadow hover:bg-blue-700">Save Free Link & Price</button>
                          </div>
                      )}
                      {activeTab === 'CONTENT_PDF_PREMIUM' && (
                          <div className="space-y-6">
                              <div>
                                  ...
                                  <div className="flex items-center bg-white border border-purple-200 rounded-xl overflow-hidden">
                                      <div className="bg-purple-100 p-3"><Link size={16} className="text-purple-600" /></div>
                              ...
                              <button onClick={saveChapterContent} className="w-full bg-purple-600 text-white font-bold py-3 rounded-xl shadow hover:bg-purple-700">Save Premium Link & Price</button>
                          </div>
                      )}
                      {activeTab === 'CONTENT_PDF_ULTRA' && (
                          <div className="space-y-6">
                              <div>
                                  ...
                                  <div className="flex items-center bg-white border border-amber-200 rounded-xl overflow-hidden">
                                      <div className="bg-amber-100 p-3"><Link size={16} className="text-amber-600" /></div>
                                      <input type="text" value={editConfig.ultraLink || ''} onChange={e => setEditConfig({...editConfig, ultraLink: e.target.value})} className="flex-1 p-3 outline-none text-sm" placeholder="https://..." />
                                  </div>
                              </div>
                              <button onClick={saveChapterContent} className="w-full bg-amber-600 text-white font-bold py-3 rounded-xl shadow hover:bg-amber-700">Save Ultra Link & Price</button>
                          </div>
                      )}
