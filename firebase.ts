...
export const saveChapterData = async (key: string, data: any) => {
  try {
    // RTDB: Use update to merge fields (prevent overwrite) if data is object
    if (typeof data === 'object' && data !== null) {
        await update(ref(rtdb, `content_data/${key}`), data);
    } else {
        await set(ref(rtdb, `content_data/${key}`), data);
    }
    
    // Firestore: Use merge: true
    await setDoc(doc(db, "content_data", key), data, { merge: true });
  } catch (error) {
